...
public class Dates {
            ...
            App.applicationHandler.post(new Runnable() {
                ...
                public void run() {
                    if (!formatterMonth.getTimeZone().equals(TimeZone.getDefault())) {
                        Dates.getInstance().recreateFormatters();
                    }
                }
            });
        }
    ...
    public static Dates getInstance() {
        Dates localInstance = Instance;
        if (localInstance == null) {
            synchronized (Dates.class) {
                localInstance = Instance;
                if (localInstance == null) {
                    Instance = localInstance = new Dates();
                }
            }
        }
    ...
    public Dates() {
        ...
        Collections.sort(sortedLanguages, new Comparator<LocaleInfo>() {
            @Override
            public int compare(Dates.LocaleInfo o, Dates.LocaleInfo o2) {
                return o.name.compareTo(o2.name);
            }
        });

        defaultLocalInfo = localeInfo = new Dates.LocaleInfo();
        ...
        is24HourFormat = DateFormat.is24HourFormat(App.applicationContext);
        LocaleInfo currentInfo = null;
        boolean override = false;

        try {
            SharedPreferences preferences = App.applicationContext.getSharedPreferences("mainconfig", Activity.MODE_PRIVATE);
    ...
    public void applyLanguage(LocaleInfo localeInfo, boolean override, boolean fromFile) {
        ...
        try {
            ...
            if (newLocale != null) {
                ...
                App.applicationContext.getResources().updateConfiguration(config, App.applicationContext.getResources().getDisplayMetrics());
    ...
    public static String formatPluralString(String key, int plural) {
        ...
        param = key + "_" + param;
        int resourceId = App.applicationContext.getResources().getIdentifier(param, "string", App.applicationContext.getPackageName());
    ...
    public String stringForMessageListDate(long date) {
        try {
            Calendar rightNow = Calendar.getInstance();
            int day = rightNow.get(Calendar.DAY_OF_YEAR);
            int year = rightNow.get(Calendar.YEAR);
